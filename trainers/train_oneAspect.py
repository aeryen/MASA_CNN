...
from networks.cnn_oneAspect import BetterCNN
...
tf.flags.DEFINE_integer("aspect_code", 5, "aspect number 0-5")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        with tf.name_scope('accuracy_summary'):
            overall_acc_summary = tf.scalar_summary("accuracy/overall-accuracy", cnn.accuracy)

        # Train Summaries
        with tf.name_scope('train_summary'):
            train_summary_op = tf.merge_summary([loss_summary, overall_acc_summary,
        ...
        with tf.name_scope('dev_summary'):
            dev_summary_op = tf.merge_summary([loss_summary, overall_acc_summary,
                                               pred_ratio_summary])
        ...
        def train_step(x_batch, y_overall_batch):
            """
        ...
        def dev_step(x_batch, y_overall_batch, writer=None):
            """
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.evaluate_every == 0:
                ...
                dev_batches = DataHelperHotelAll \

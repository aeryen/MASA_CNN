...
def write_hyperparams(out_dir, FLAGS, l2_reg=-1, num_filter=-1, drop_k=-1):
    ...
    hyper_param_file.write("\n\n\n")
    hyper_param_file.write("l2_reg: " + str(l2_reg))
    hyper_param_file.write("num_filter: " + str(num_filter))
    hyper_param_file.write("dropout_keep_rate: " + str(drop_k))

    hyper_param_file.close()


def train_one_nn(x_train, x_dev, y_train, y_dev, vocabulary, sc_train, sc_dev, embed_matrix, l2_reg, num_filter, drop_k):
    # Training
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", "my_beer", "regression2-2",
                                                   timestamp + "_" + str(num_filter)))
            print("Writing to {}\n".format(out_dir))
            write_hyperparams(out_dir=out_dir, FLAGS=FLAGS, l2_reg=l2_reg, num_filter=num_filter, drop_k=drop_k)
...
if __name__ == "__main__":
    ...
    # test_x, test_y, _, _, s_count = data_helpers_allAspect_glove_beer.load_test_data()
    ...
    for l2 in [0.05]:
        for filter_size in [100]:
            for drop_out_keep in [0.75]:
                train_one_nn(x_train, x_dev, y_train, y_dev, vocabulary, sc_train, sc_dev, embed_matrix,
                             l2_reg=l2, num_filter=filter_size, drop_k=drop_out_keep)

...
from networks.cnn_multiAspect_a2a_ratesway import BetterCNN
...
tf.flags.DEFINE_float("l2_reg_lambda", 1.5, "L2 regularizaion lambda (default: 0.0)")
...
time1 = time.time()
x, y_overall, y_aspect, vocabulary, vocabulary_inv = data_helpers_allAspect_sway.load_data()
time2 = time.time()
...
np.random.seed(10)
shuffle_indices = np.random.permutation(np.arange(len(y_overall)))
x_shuffled = x[shuffle_indices]
y_overall_shuffled = y_overall[shuffle_indices]
y_aspect_shuffled = y_aspect[shuffle_indices]

# Split train/test set
# TODO: This is very crude, should use cross-validation
x_train, x_dev = x_shuffled[:-1000], x_shuffled[-1000:]
y_overall_train, y_overall_dev = y_overall_shuffled[:-1000], y_overall_shuffled[-1000:]
y_aspect_train, y_aspect_dev = y_aspect_shuffled[:-1000], y_aspect_shuffled[-1000:]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(x_batch, y_overall_batch, y_aspect_batch):
            """
            A single training step
            """
            feed_dict = {
                cnn.input_x: x_batch,
                cnn.input_y_all: y_overall_batch,
                cnn.input_y_aspect: y_aspect_batch,
        ...
        def dev_step(x_batch, y_overall_batch, y_aspect_batch, writer=None):
            """
        ...
        # Generate batches
        batches = data_helpers_allAspect_sway.batch_iter(
            list(zip(x_train, y_overall_train, y_aspect_train)), FLAGS.batch_size, FLAGS.num_epochs)
        # Training loop. For each batch...
        for batch in batches:
            x_batch, y_overall_batch, y_aspect_batch = list(zip(*batch))
            train_step(x_batch, y_overall_batch, y_aspect_batch)
            current_step = tf.train.global_step(sess, global_step)
            if current_step % FLAGS.evaluate_every == 0:
                print("\nEvaluation:")
                dev_batches = data_helpers_allAspect_sway \
                    .batch_iter(list(zip(x_dev, y_overall_dev, y_aspect_dev)), 64, 1)
                for dev_batch in dev_batches:
                    small_dev_x, small_dev_y_all, small_dev_y_aspect = list(zip(*dev_batch))
                    dev_step(small_dev_x, small_dev_y_all, small_dev_y_aspect, writer=dev_summary_writer)
                    print("")

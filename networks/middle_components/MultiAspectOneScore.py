...
class BetterCNN(object):
    ...
    def __init__(
            self, previous_component, document_length, sequence_length, num_aspects, num_classes,
            vocab_size, embedding_size, filter_size_lists, num_filters, l2_reg_lambda=0.0,
            init_embedding=None):
        ...
        num_filters_total = num_filters * len(filter_size_lists)
        ...
        with tf.name_scope("dropout-keep" + str(0.5)):
            ...
            # [batch_size, sentence * num_filters_total]
            self.h_drop_review = tf.reshape(self.h_drop_sentence, [-1, document_length * num_filters_total],
                                            name="h_drop_review")
        ...
        with tf.name_scope("related"):
            W = tf.get_variable(
                "W_r",
                shape=[num_filters_total, num_aspects + 1],
                initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.name_scope("output"):
            for aspect_index in range(num_aspects + 1)[1:]:

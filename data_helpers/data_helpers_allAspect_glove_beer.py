...
def load_data_and_labels():
    ...
    train_count = list(open("./data/beer_100k/train.count", "r").readlines())
    train_count = [int(s) for s in train_count if (len(s) > 0 and s != "\n")]

    train_rating = list(open("./data/beer_100k/train.rating", "r").readlines())
    train_rating = [s for s in train_rating if (len(s) > 0 and s != "\n")]
    y = [[identity[int(float(i) * 2)] for i in s.split(" ") if (len(i) > 0 and i != "\n")] for s in train_rating]

    train_content = list(open("./data/beer_100k/train.txt", "r").readlines())
...
def load_test_data_and_labels():
    ...
    # Load data from files
    train_count = list(open("./data/beer_100k/test.count", "r").readlines())
    train_count = [int(s) for s in train_count if (len(s) > 0 and s != "\n")]

    train_rating = list(open("./data/beer_100k/test.rating", "r").readlines())
    train_rating = [s for s in train_rating if (len(s) > 0 and s != "\n")]
    y = [[identity[int(float(i) * 2)] for i in s.split(" ") if (len(i) > 0 and i != "\n")] for s in train_rating]

    train_content = list(open("./data/beer_100k/test.txt", "r").readlines())
    train_content = [s.strip() for s in train_content]
...
def load_glove_vector():
    ...
    glove_vectors = np.array(vector_list).astype(float)
...
def build_embedding(vocabulary_inv, glove_words, glove_vectors):
    ...
    for word in vocabulary_inv:
        ...
        if word == "<PAD/>":
            embed_matrix.append(np.zeros(100))
        else:

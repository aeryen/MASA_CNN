...
tf.flags.DEFINE_string("checkpoint_dir", "./runs/1468649122/checkpoints/", "Checkpoint directory from training run")
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        batch_review_aspect_score = graph.get_operation_by_name("output/output_score_sum").outputs[0]
        ...
        for x_test_batch, y_test_batch in zip(x_batches, y_batches):
            ...
            [rate_score, prob_sum, sent_relate] = sess.run([batch_review_aspect_score, review_aspect_prob_sum,
                                                            sent_relate_un_norm],
                                                           {input_x: x_test_batch, input_y: y_test_batch,
                                                            dropout_keep_prob: 1.0})
            review_final_value = tf.div(tf.transpose(rate_score), prob_sum).eval()
            all_prediction.append(review_final_value)

            sent_relate_max = tf.argmax(sent_relate, 1).eval()
            for sent_index in range(rate_score.shape[1]):
...
np.savetxt('aspect_rating.out', all_prediction, fmt='%1.0f')
